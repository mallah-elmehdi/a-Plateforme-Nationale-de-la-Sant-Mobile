$(document).ready(function () {
	...
	var wholeData = $('#data').data('provincedata'),
		...
		// DATA LOADING
		data = getData(
			[wholeData.femmeExamineCancerSein, wholeData.femmeExamineCancerCol],
			csrList
		),
		extraText = '';
	// CHART OPTION
	...
	$('input[type=radio][name=data]').change(function () {
		...
		if (this.value === 'femmeExamine') {
			data = getData(
				[
					wholeData.femmeExamineCancerSein,
					wholeData.femmeExamineCancerCol,
				],
				csrList
			);
			table = 2;
			...
			data = getData(
				[
					wholeData.femmeRefereCancerSein,
					wholeData.femmeRefereCancerCol,
				],
				csrList
			);
			table = 2.1;
