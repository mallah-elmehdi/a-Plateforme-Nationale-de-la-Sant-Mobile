$(document).ready(function () {
	...
	function pdfListner() {
		...
		function generateMulPdf(header, direction,delegation, year) {
			...
			doc.text('Trimestre : ' + trimestre, 15, 72);
			doc.text('Année : ' + year, 15, 78);
			// ADD ALL TABLES
	...
	function autreActiviteData(autreActivite) {
		var out = '';
		for  (const key1 in autreActivite) {
				const commune = autreActivite[key1];
				out += `<tr><td rowspan="${commune.length}">${key1}</td>
		...
		if (out === '') {
			out =
				'<tr><td>Aucune</td><td>Aucune</td><td>0</td><td>Aucune</td></tr>';
		}
		return out;
	}
	// ADD PDR
	function pdrVisiteData(pdrVisite) {
		...
		if (out === '') {
			out =
				'<tr><td>Aucune</td><td>Aucune</td><td>0</td><td>0</td></tr>';
		}
		return out;
	}
	// SHOW TABLE
	function showModal(code) {
		...
		title = 'Rapport de la province ' + pro + ' - Trimestre ' + trimestre;
			...
			<div class="modal-dialog modal-fullscreen">
				<div class="modal-content">
					...
					<div class="modal-body bg-body">
						<div class="row g-3">
							<div class="col-12">
								<button type="button" class="btn btn-sm btn-light downloadPdf"  data-header='Rapport de suivi des activités de l’équipe mobile' data-region='${reg}' data-province='${pro}'>
