$(document).ready(function () {
	...
	function generateMulPdf(region, province, csr, category, trimestre, createdat, updatedat, submit) {
		// VARIABLES
		var listTable = [];
		// GET ALL TABLES
		...
		doc.text('Fait le : ' + createdat.getDate() + '/' + createdat.getMonth() + '/' + createdat.getFullYear(), 15, 90);
		if (submit)
			doc.text('Soumis le : ' + updatedat.getDate() + '/' + updatedat.getMonth() + '/' + updatedat.getFullYear(), 15, 96);
		else
			doc.text('Soumis le : Non soumis', 15, 96);
		// ADD ALL TABLES
		for (let i = 0; i < listTable.length; i++) {
			...
			if (i != 0) {
				...
				doc.text(i + 1 + '- ' + listTable[i].title, 15, 106);
				doc.autoTable({
					html: '#' + listTable[i].id,
					startY: 109,
					theme: 'grid',
	...
	// call the function
	$('#downloadSortieAllPdf').click(function () {
		// DOWNLOAD ALL PDFS
		generateMulPdf(
			$(this).data('region'),
			$(this).data('province'),
