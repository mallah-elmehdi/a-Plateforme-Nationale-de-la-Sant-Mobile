$(document).ready(function () {
	...
	function generateMulPdf(region, province, csr, category, createdat, updatedat, submit) {
		// VARIABLES
		var listTable = [];
		// GET ALL TABLES
		...
		doc.setFontSize(15);
		// INSERT THE HEADER
		doc.text('Plan d’action des unités médicales mobiles (UMM)', 50, 50);
		...
		if (submit)
			doc.text('Soumis le : ' + updatedat.getDate() + '/' + updatedat.getMonth() + '/' + updatedat.getFullYear(), 15, 90);
		else
			doc.text('Soumis le : Non soumis', 15, 90);
		// ADD ALL TABLES
		for (let i = 0; i < listTable.length; i++) {
			...
			if (i != 0) {
				...
				doc.text(i + 1 + '- ' + listTable[i].title, 15, 100);
				doc.autoTable({
					html: '#' + listTable[i].id,
					startY: 103,
					theme: 'grid',
		...
		doc.save(
			"plan d'action des unités médicales mobiles (umm) - " + csr + ' - ' + createdat.getFullYear() + '.pdf'
		);
	}
	// call the function
