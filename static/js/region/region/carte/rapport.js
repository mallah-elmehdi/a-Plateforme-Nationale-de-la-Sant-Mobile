$(document).ready(function () {
	...
	$('#region-map').vectorMap({
		...
		onRegionClick: function (e, code) {
			if (code == codeRegion) showModal(code);
			else showErrorModal();
		},
	});
	// obj size
	...
	function autreActiviteData(autreActivite) {
		...
		if (out === '') {
			out =
				'<tr><td>Aucune</td><td>Aucune</td><td>Aucune</td><td>0</td><td>Aucune</td></tr>';
		}
		return out;
	}
	// ADD PDR
	function pdrVisiteData(pdrVisite) {
		...
		if (out === '') {
			out =
				'<tr><td>Aucune</td><td>Aucune</td><td>Aucune</td><td>0</td><td>0</td></tr>';
		}
		return out;
	}
	// SHOW TABLE
	function showModal(code) {
		var title = 'Rapport de la région ' + region + ' - Trimestre ' + trimestre;
			...
			<div class="modal-dialog modal-fullscreen">
				<div class="modal-content">
					...
					<div class="modal-body bg-body">
						<div class="row g-3">
							<div class="col-12">
								<button type="button" class="btn btn-sm btn-light downloadPdf"  data-header='Rapport de suivi des activités de l’équipe mobile' data-region='${region}'>
