...
async function changePassword(req, res, next) {
	try {
		...
		// render the page
		res.status(200).render('region/changePassword', {
			title: data.document.region + ' | Changer le mot de passe',
			data,
		});
	} catch (error) {
		console.log(error);
...
async function checkOldPassword(req, res, next) {
	try {
		// get the document
		var currentUser = await regionData.getDocument(req.params.id);
		// get the candidate
